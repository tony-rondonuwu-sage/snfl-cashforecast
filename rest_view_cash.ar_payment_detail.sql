...
CREATE OR REPLACE VIEW accounts_payable_payment_detail
  WITH ROW ACCESS POLICY MAIN_DBACCT_DB.SIF_REPLICATION_SCHEMA.ENFORCE_COMPANY_ACCESS ON (accounts_payable_payment_detail.CNY_)
  AS
SELECT
    accounts_payable_payment_detail.DISCOUNTDATE AS discountDate,
    accounts_payable_payment_detail.RECORDNO AS id,
    accounts_payable_payment_detail.RECORDNO AS key,
    ...
    audit_createdByUser.loginid AS CREATEDBYLOGINID,
    audit_modifiedByUser.loginid AS MODIFIEDBYLOGINID,
    ...
    accounts_payable_payment_detail.jointpayee.record# AS jointPayee_id,
    accounts_payable_payment_detail.jointpayee.jointpayeeprintas AS jointPayee_printAs
FROM
    ICRW_SCHEMA.pymtdetail accounts_payable_payment_detail
LEFT JOIN ICRW_SCHEMA.userinfo audit_createdByUser ON audit_createdByUser.CNY_ = accounts_payable_payment_detail.CNY_ AND audit_createdByUser.record_ = accounts_payable_payment_detail.createdby
LEFT JOIN ICRW_SCHEMA.userinfo audit_modifiedByUser ON audit_modifiedByUser.CNY_ = accounts_payable_payment_detail.CNY_ AND audit_modifiedByUser.record_ = accounts_payable_payment_detail.modifiedby
...
FROM
    ICRW_SCHEMA.v_recrprrecschdleop accounts_receivable_recurring_invoice
...
CREATE OR REPLACE VIEW accounts_receivable_payment_detail AS
SELECT
    -- Main fields
    accounts_receivable_payment_detail.DISCOUNTDATE AS discountDate,
    accounts_receivable_payment_detail.__framework__ AS href,
    accounts_receivable_payment_detail.RECORDNO AS id,
...
FROM ICRW_SCHEMA.pymtdetail accounts_receivable_payment_detail
-- Audit joins
LEFT JOIN ICRW_SCHEMA.userinfo audit_createdByUser ON audit_createdByUser.CNY_ = accounts_receivable_payment_detail.CNY_ AND audit_createdByUser.record_ = accounts_receivable_payment_detail.createdby
LEFT JOIN ICRW_SCHEMA.userinfo audit_modifiedByUser ON audit_modifiedByUser.CNY_ = accounts_receivable_payment_detail.CNY_ AND audit_modifiedByUser.record_ = accounts_receivable_payment_detail.modifiedby
;
...
SELECT
    ...
    accounts_receivable_recurring_invoice_line.__framework__ AS href,
