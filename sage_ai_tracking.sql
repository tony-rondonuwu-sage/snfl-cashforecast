...
CREATE OR REPLACE STREAM cf_enabled_st_customer ON VIEW cf_enabled_customer;
...
BEGIN
    -- ========== PRRECORD ==========
    BEGIN TRANSACTION;
    INSERT INTO cf_upsert_records (cny_, record_, tablename, recordtype, operation_type)
    SELECT 
        m1.cny_, 
        m1.record_, 
        'PRRECORD',
    ...
    FROM cf_enabled_st_prrecord m1
    WHERE m1.METADATA$ACTION = 'INSERT';
    prrecord_upsert_count := SQLROWCOUNT;

    INSERT INTO cf_delete_records (cny_, record_, tablename, recordtype)
    SELECT 
        m1.cny_,
        m1.record_,
        'PRRECORD',
    ...
    -- ========== CUSTOMER ==========
    BEGIN TRANSACTION;
    INSERT INTO cf_upsert_records (cny_, record_, tablename, operation_type)
    SELECT 
        m1.cny_, 
        m1.record_, 
        'CUSTOMER',
        CASE WHEN m1.METADATA$ISUPDATE = TRUE THEN 'U' ELSE 'I' END
    FROM cf_enabled_st_customer m1
    WHERE m1.METADATA$ACTION = 'INSERT';
    customer_upsert_count := SQLROWCOUNT;

    INSERT INTO cf_delete_records (cny_, record_, tablename)
    SELECT 
        m1.cny_,
        m1.record_,
        'CUSTOMER'
    ...
    -- ========== TERM ==========
    BEGIN TRANSACTION;
    INSERT INTO cf_upsert_records (cny_, record_, tablename, module, operation_type)
    SELECT 
        m1.cny_, 
        m1.record_, 
        'TERM',
        m1.modulekey,
        CASE WHEN m1.METADATA$ISUPDATE = TRUE THEN 'U' ELSE 'I' END
    FROM cf_enabled_st_term m1
    WHERE m1.METADATA$ACTION = 'INSERT';
    term_upsert_count := SQLROWCOUNT;

    INSERT INTO cf_delete_records (cny_, record_, tablename, module)
    SELECT 
        m1.cny_,
        m1.record_,
        'TERM',
    ...
    SELECT 
        ...
        'LOCATION',
        CASE WHEN m1.METADATA$ISUPDATE = TRUE THEN 'U' ELSE 'I' END
    FROM cf_enabled_st_location m1
    WHERE m1.METADATA$ACTION = 'INSERT';
