----------------------------------------------------------------------------
-- Clean up incorrectly created task, streams and table

DROP TASK MAIN_DBACCT_DB.SIF_SAGE_VIEWS.capture_deleted_records;
DROP STREAM MAIN_DBACCT_DB.SIF_SAGE_VIEWS.st_prrecord_delete;
DROP STREAM MAIN_DBACCT_DB.SIF_SAGE_VIEWS.st_prentry_delete;
DROP STREAM MAIN_DBACCT_DB.SIF_SAGE_VIEWS.st_customer_delete;
DROP STREAM MAIN_DBACCT_DB.SIF_SAGE_VIEWS.st_term_delete;
DROP STREAM MAIN_DBACCT_DB.SIF_SAGE_VIEWS.st_location_delete;
-- The following table is shared, is it okay to drop?
DROP TABLE MAIN_DBACCT_DB.SIF_SAGE_VIEWS.deleted_record;

--Looks like Calin created these and no longer used
--DROP STREAM MAIN_DBACCT_DB.CASH_FORECASTING_VIEWS_MAIN.PRRECORD_STREAM;
--DROP STREAM MAIN_DBACCT_DB.CASH_FORECASTING_VIEWS_MAIN.SCHEDULE_STREAM;


----------------------------------------------------------------------------
-- Now doing the actual migration

use database MAIN_DBACCT_DB;

use schema MAIN_DBACCT_DB.CASH_FORECASTING_VIEWS_MAIN;
...
---- COPIED from deletedrecords.sql
...
-- Next steps:
-- 1. Change the schedule of the "capture_deleted_records" task to every hour on the 1 minute of the hour (CRON: 1 */4 * * *)
-- 2. Add the new AP views to the share, as well as the new DELETED_RECORD table
